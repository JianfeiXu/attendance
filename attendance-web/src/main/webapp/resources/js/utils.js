 ...
 * @param tabCode		:表代码
 * @param repVal		:替换值
 * @param repDomId		:替换节点id(替换下拉框id)
 * @param defaultVal	:repVal为空时替换节点默认显示
 * @param contextPath	:环境上下文(一般通过request.getContextPath获取)
 */
function dynamicFillSelect(tabCode, repVal, repDomId, defaultVal, contextPath) {
	var repObj = $("#" + repDomId);
	if("" != repVal) {
		$.ajax({
			type: "POST",
			url: contextPath + "/stusv/eplatform/module/common/dict/util/loadByTabCode.action",
			data:{
				tabCode : tabCode,
				repVal  : repVal
			},
			...
			success: function(msg) {
				...
				repObj.append("<option selected value=''>--请选择--</option>");
				if($(objlist).size() > 0) {
					for(var i = 0; i < $(objlist).size(); i++){
						repObj.append("<option value='"+objlist[i].code+"'>" + objlist[i].name + "</option>");
					}
				}
				
				repObj.change(); //触发change事件
			},
			error: function() {
		...
		repObj.empty();
		repObj.append("<option selected value=''>" + defaultVal + "</option>");
		repObj.change(); //触发change事件
	}
}
